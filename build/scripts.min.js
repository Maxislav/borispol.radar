...
})},navTabs:function(n){$(".nav.nav-tabs").find("li").removeClass("active"),$(".nav.nav-tabs").find("li").eq(n).addClass("active")},mask:{show:function(el){el?(el.append('<div class="mask"><svg width="60" height="60" class="svg"><image xlink:href="img/loader.png" width="60" height="60"><animateTransform attributeType="xml"attributeName="transform" type="rotate"from="0 30 30"to="360 30 30"dur="4s"repeatCount="indefinite"/></image></svg></div>'),el.find(".mask").height(el.height()).fadeTo(222,1),el.find(".mask").css({position:"absolute"}),el.find(".svg").css({position:"absolute",left:el.width()/2-30+"px",top:el.height()/2-30+"px"})):($("body").append('<div class="mask"><svg width="60" height="60" class="svg"><image xlink:href="img/loader.png" width="60" height="60"><animateTransform attributeType="xml"attributeName="transform" type="rotate"from="0 30 30"to="360 30 30"dur="4s"repeatCount="indefinite"/></image></svg></div>'),$(".mask svg").click(function(){app.mask.hide()}),$(".mask").height($(window).height()).fadeTo(222,1),$(".svg").css({position:"absolute",left:$(window).width()/2-30+"px",top:$(window).height()/2-30+"px"}))},hide:function(el){el?el.find(".mask").fadeTo(222,0,function(){$(this).remove()}):$(".mask").fadeTo(222,0,function(){$(this).remove()})}}},timer=new Timer,main=new Main;